 ...
 *   npx github:go-code-bot/list-builder add ./path
 *   npx github:go-code-bot/list-builder sync ./path
 */
  ...
  return new Promise((resolve, reject) => {
    https.get(url, {
      headers: {
        ...
        'User-Agent': 'list-builder-cli/1.0',
      },
    }, (res) => {
...
function fetchBuffer(url) {
  return new Promise((resolve, reject) => {
    https.get(url, {
      headers: { 'User-Agent': 'list-builder-cli/1.0' },
    }, (res) => {
      if (res.statusCode === 301 || res.statusCode === 302) {
...
async function installSkills(targetDir, { overwrite = false } = {}) {
  ...
  console.log(`\nInstalled list-builder skill files: ${written} written, ${skipped} skipped.`);
  console.log('Validate with: node scripts/validate-list.js assets/list.json');
}
